...
### 合并后修复轮（2026-09-01，PR #1 合并后 master 三笔加固）

- **裁决 2 —— 日线缺省轴解析不再与日内同构（commit dcf92a6）**：
  - **冲突**：PR 合并时的设计注记称 `_remap_daily_axes` "面板0 原样返回，与日内
    `_remap_axes` 同构"。修复后日线**所有面板（含面板0/单面板）**缺省轴解析到
    本面板主轴，刻意的同构被打破。
  - **裁决**：两产品线在 panel[0] 上本质不同——日内 panel[0] 缺省 y2 是贴水
    overlay（真实存在、语义正确）；日线无 overlay，缺省 y2 在多面板下恰是
    第 2 行量轴（手写层静默画错面板）、单面板下是幽灵轴（不可见/守卫误报
    "副轴请给 from/to"）。缺省解析必须落在产品线上下文已知的渲染层，而非
    原语层硬编码。"同构"保留在机制层面（缺省=本面板轴、字面 y 改写、显式
    轴引用不改写），差异只在 panel[0] 的缺省目标。
  - **证据**：3 回归修前全败/修后全过；插件自产层 setdefault(axis=y) 使 remap
    no-op，gate 14 场景 0 diff（既有渲染零影响）。
- **P2#4 包络不变式闭合（commit 9d52c2e，修入 PR 后合并）**：包络分支
  `min(b0-10%span)` 在 `0.1*span > b0+15` 的 leak 区突破 -15，与 PR 自述
  "包络内逐值不变"矛盾；改为包络分支钉死 -15、下扩只属超包络分支，
  leak 区 5 例参数化钉死（原钉死测试 fixture 恰不在 leak 区，未覆盖该不变式）。
- **日内 row_heights 校验对齐（commit 723ba67）**：多面板原报 plotly 英文错、
  **单面板被 `_build_single` 静默丢弃**；现与日线同文案中文校验（双路径回归）。
