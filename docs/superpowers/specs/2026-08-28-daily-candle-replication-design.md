...
**元素 ↔ 图层总账（四幅参考图目视盘点）**——视觉元素共 9 类，对应图层类型 9 种（新增 6 + 复用 3）；轴刻度/网格/脚注属图框架（FR-4），不计标注元素。

| # | 视觉元素 | 分布 | 图层原语 | 状态 |
|---|---|---|---|---|
| 1 | 蜡烛图（红涨青跌+影线） | 4 幅全有 | `candle` | 新增 |
| 2 | 多周期均线（每幅 4–5 条） | 4 幅全有 | `line` | 复用 |
| 3 | 水平支撑/压力/分界线 | 01×3、02×3、03×2、05×1~2 | `hline` | 复用 |
| 4 | 趋势线/通道线（实/虚、成对通道） | 4 幅全有 | `trendline` | 新增 |
| 5 | 矩形区（红实框/红虚框集合区） | 仅 05（×3） | `zone` | 复用 |
| 6 | 圆圈标记（可带①②③④序号） | 02、05 | `circle` | 新增 |
| 7 | 箭头（竖直/斜向/走势预演V形分叉） | 4 幅全有 | `arrow` | 新增 |
| 8 | 自由彩字（大字标注/小字/高低点价签） | 4 幅全有 | `text` | 新增 |
| 9 | 右缘药丸标签（价格/BULL/BEAR/BASE） | 4 幅全有 | `tag` | 新增 |

一期样板 05 的图层实例构成（**约 31 条** = 插件默认 6 条 + YAML 标注约 25 条）：行情层 6（蜡烛+5均线）｜线形 4（白趋势线、黄虚通道×2、7650压力线）｜区域 3（筹码密集区实框、左右集合区间虚框）｜圆圈 4（①②③④）｜引线 5（区间宽度560/540箭头、BULL斜箭头、BEAR绿V、BASE黄V）｜文字 6（品种大字、筹码密集区、高低点价签、BULL/BEAR/BASE）｜右缘标签 3（7560/8098.8/6450.9）。
