...
- [x] **Step 2: 跑测试确认失败** → `pytest tests/test_signals.py::test_event_meta_default_none -q` FAIL（`TypeError: unexpected keyword 'meta'`）

- [x] **Step 3: 改 dataclass 一行**
...
- [x] **Step 4: 全量回归** → `pytest -q` 全绿（既有测试构造 Event 均不传 meta，向后兼容）
- [x] **Step 5: Commit** `git add -A && git commit -m "feat(core): Event 增加 meta 字段（动作信息承载，向后兼容）"`
...
- [x] **Step 2: 确认失败** → `pytest tests/test_position.py -q` FAIL（模块不存在）
...
- [x] **Step 4: 通过** → `pytest tests/test_position.py -q` → `4 passed`
- [x] **Step 5: Commit** `git add -A && git commit -m "feat(core): expand_trades 交易明细展开为仓位列与执行事件"`
...
- [x] **Step 2: 确认失败** → FAIL（新字段无校验，前两测试应抛而未抛）

- [x] **Step 3: config.py 追加校验（`return cfg` 之前）**
...
- [x] **Step 4: 通过** → `pytest tests/test_pipeline.py -q` 全绿
- [x] **Step 5: Commit** `git add -A && git commit -m "feat(config): trades/trades_csv/extra_panels/contract_mult 校验（错误定位到条目字段）"`
...
- [x] **Step 2: 确认 merge 测试失败** → `pytest tests/test_pipeline.py::test_merge_panels_priority -q` FAIL（参数不匹配）

- [x] **Step 3: 改 pipeline.py**
...
- [x] **Step 4: merge 测试通过；全量回归** → `pytest tests/test_pipeline.py::test_merge_panels_priority -q` PASS；`pytest -q` 除 `test_pipeline_trades_and_extra_panels`（待 Task 6）外全绿
- [x] **Step 5: Commit**（只提交通过的部分：将 `test_pipeline_trades_and_extra_panels` 暂标 `@pytest.mark.skip(reason="待 Task 6 多面板")`，Task 6 解除）`git add -A && git commit -m "feat(core): pipeline 接入 trades 展开与 extra_panels 追加"`
...
- [x] **Step 2: 确认失败** → FAIL（`line.shape` 不存在 / marker.symbol 仍是默认）
...
- [x] **Step 4: 通过 + 全量回归** → `pytest tests/test_primitives_events.py -q`；`pytest -q`（无行为变化：不带 style_map/shape 时输出同旧）
- [x] **Step 5: Commit** `git add -A && git commit -m "feat(render): line 阶梯shape透传 + events 按动作style_map"`
...
- [x] **Step 2: 确认失败** → FAIL（现 build_figure 断言 len==1）

- [x] **Step 3: 改 figure.py**——现 `build_figure` 主体原样平移为 `_build_single(df, slots, panel, rep, title)`（第一个参数改单 panel，循环删除；其余一字不动），新入口与多面板实现：
...
- [x] **Step 4: 通过 + 解禁 Task 4 的 skip 测试 + 全量回归**

Run: `pytest tests/test_figure.py -q` → PASS；删除 Task 4 的 `@pytest.mark.skip` 后 `pytest -q` 全绿
- [x] **Step 5: Commit** `git add -A && git commit -m "feat(render): 多面板框架（N==1原路径零改动；面板0贴水overlay轴重映射；row_heights可配）"`
...
- [x] **Step 2: 确认失败** → FAIL（common 不存在）

- [x] **Step 3: 建 common.py；excel_wind 改薄**
...
- [x] **Step 4: 通过 + 既有 excel 测试全绿** → `pytest tests/test_common_align.py tests/test_excel_wind.py -q`
- [x] **Step 5: Commit** `git add -A && git commit -m "refactor(adapters): 对齐层下沉 common.py（excel_wind 与 local_ds 同源）"`
...
- [x] **Step 1: 写失败测试（含 fake local_datasource 注入装置）**
...
- [x] **Step 4: 通过** → `pytest tests/test_local_ds.py -q` → `3 passed`
- [x] **Step 5: Commit** `git add -A && git commit -m "feat(adapters): local-datasource 适配器（CSV读回/附加列忽略/CoverageGap转译）"`
...
- [x] **Step 2: 确认失败** → FAIL（现 auto/api 直接抛二期报错）
...
- [x] **Step 4: 通过 + 全量** → `pytest tests/test_api_auto.py -q`；`pytest -q` 全绿
- [x] **Step 5: Commit** `git add -A && git commit -m "feat(adapters): auto/api 接入 local-datasource（覆盖缺口→Excel回退/指引，不静默）"`
...
- [x] **Step 7: Commit** `git add -A && git commit -m "feat: 二期收尾（契约测试/联调marker/示例配置/e2e验收/README）"`
