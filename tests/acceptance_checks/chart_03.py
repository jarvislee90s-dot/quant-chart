...
L2 相对位置：药丸与水平线同高（±3）；下行通道上轨压住窗口最高 117.59（2025-09-04，±5）、右端 ≈117
        与"背靠通道上轨117"吻合；下轨托 26-01-08 低点 110.40 并贴 26-03-23 窗口最低 110.31（±5）；
        「上轨117」文字紧邻通道上轨（±10，且在轨上方）；药丸与水平线同高（±3）；多空分割线 113.40
        位于上下轨之间；两条红竖箭头各落在相邻两水平线所夹带内；蓝字在其对应箭头夹带内；
        BULL/BEAR 文字锚定最后K线（241 根，pos 240）右侧预测区、其箭头头部入预测区（尾部可起自
...
L3 数学：通道斜率 < 0 且 == fit_channel 斜率（±1e-6）；两轨平行（±1e-6）；渲染==拟合（±3）；
  ...
  窗口差异说明：样张为 24-09~26-08 两年窗（x 轴合约切换非线性压缩），本图为其后半段一年窗
        （241 根）；样张 122.28 顶药丸/108.61 低点价签在窗外不复刻；样张灰白慢均线（需窗外数据）
...
PEAK_HIGH, WINDOW_LOW = 117.59, 110.31                        # 数据窗口最高（25-09-04）/最低（26-03-23）
...
def run(fig, df, rep, cfg) -> Verifier:
    ...
    fit = fit_channel(d, "2025-09-01", "2026-08-28", tilt=0.12, press=1.0)
    if not fit.slope < 0:
