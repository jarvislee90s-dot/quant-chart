...
def make(path: str, code: str, days, base: float, drop_minutes: int = 0,
    ...
    if zero_vol_minute:                    # 模拟零成交分钟（驱动 vwap 前值填充分支）
        df.loc[df["日期"].dt.strftime("%H:%M") == zero_vol_minute, "成交量(股)"] = 0
...
if __name__ == "__main__":
    ...
    make(out / "fut.xlsx", "IM2612.CFE", days, 7000.0, zero_vol_minute="10:00")
